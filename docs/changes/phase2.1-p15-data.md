...
- 状态：**P15-01～06 全部 DONE**（数据层 + 执行层 + 面板）
...
`renderer.js` 的 `showDatabasePanel()`（动态 DOM）：左侧表列表 → 点开看列信息与分页数据；
搜索框（转义后走 LIKE）；上一页/下一页；Redis 侧一个"看 Keys"（走 SCAN）。

**最要紧的一条**：连不上时面板上显示的是**可读原因**（"未找到 psql 客户端"），
而**不是**"（public schema 里没有表）"这种会骗人的话 —— 端到端对这句专门加了断言。
另外面板路径同样过安全闸：从面板 API 直接塞 `DROP TABLE users` 或多语句，照样被拦。
