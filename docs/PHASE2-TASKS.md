...
| P13 | Workbench | **部分 PASS**：**P13-01～03 数据层**（最近项目同根去重与提序 / 项目待办增勾删 / 项目便签，全部**按项目隔离**）+ **P13-08 不反向污染**（只读项目信息、不导出改 IDE 的函数、store 只含自己的键；把 IDE 字段传进来也不会被带进 store）；`test-workbench` **56/0**；**P13-04～07（File Relay→Project / Office 预览 / Office→Agent / Agent Summary→Note）与面板未做** |
