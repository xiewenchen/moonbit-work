...
| P19 | 测试体系 | **进行中**：`verify-harness` + 元测试 36 项 + 扫描器（现**认可相等语义的 chk**）+ 异常退出码双向证明 + 4 个长期红灯已修；**但"16 个 chk 全迁移"仍未达成** —— 本批尝试批量迁移**失败并已回滚**（只删定义没插 harness → 13 个脚本报 chk 未定义；详见 `docs/changes/phase2.1-p19-migration-attempt.md`）；正确做法是**一次一个文件、改完立刻真跑**（`node --check` 查不出未定义变量） |
