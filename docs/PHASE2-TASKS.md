...
| P13 | Workbench | **部分 PASS**：数据层（最近项目/待办/便签，**按项目隔离**）+ **P13-08 不反向污染**（磁盘上只有 workbench 自己的键）+ **面板**（最近项目可点开/待办可勾删/便签）+ **P13-04～07 数据层**（文件→项目**显式关联**（也按项目隔离）/ Office 预览（**复用 relay 的元信息解析**，没元信息如实说）/ 送进 Agent（**每样都带出处**）/ 结论**追加**到便签）；`test-workbench` **56/0**、`verify-workbench` **23/0**、`test-office-link` **53/0**；**P13-04～07 的 IPC 与面板未做** |
