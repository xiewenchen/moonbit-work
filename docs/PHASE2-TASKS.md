...
| P20 | 产品化 | **部分 PASS（基础层）**：**P20-07/08 启动恢复**（正常退出→完整恢复；**异常退出→降级只恢复项目**并说明原因；快照只记"位置"不记正文）+ **P20-09 环境检查**（Moon/Node/Git/Docker/Agent，每项带**为什么需要**与**可执行建议**；**"没装"与"探测报错"分开**；没注入探测能力就明确抛）；`test-startup-state` **47/0**；**P20-01～06/10 的 UI 重排与接线未做** |
| P21 | 最终产品 Demo | TODO |
| P22 | 最终回归 | TODO |
