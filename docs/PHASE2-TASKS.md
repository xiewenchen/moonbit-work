...
| P20 | 产品化 | **PASS（除 P20-02/03 的进一步产品化）**：**P20-01 欢迎屏重排**（最近项目←复用 P13 / 新建 / 继续上次←P20 快照 / AI Agent）+ **P20-05 Settings**（Auto-start 开关真有作用、主题、数据存放位置）+ **P20-06 About**（版本来自 `app:about` IPC 真实探测，非硬编码），三项均**动态注入、不改转译产物**；另含 **P20-07/08 启动恢复**（异常退出降级）、**P20-09/10 环境检查与诊断**、**P20-04 面板入口**（顶栏按钮）；`test-startup-state` **47/0**、`verify-startup-ui` **25/0**、`verify-panel-entry` **14/0**、`verify-p20-ui` **20/0** |
| P21 | 产品演示 | **PASS（可跑的部分）**：`desktop/verify-demo-full.js` 用**真实项目 conduit**（http+pg+redis + 4 个测试文件）跑 17 步 → **28/0**；**12 步真跑**（注入错误→打开→报错进问题模型→Agent 查看/Explain/Read→Patch 生成/预览/应用→**Check 真跑并由失败恢复 0**→Quality→总结写便签），**5 步如实标"因环境没跑"**（Test/Run/Browser 受 R12、API/Database 未起服务/未装客户端，都**如实报错不假装**）；说明见 `docs/DEMO-P21.md`；**全程未改 MoonBit 业务代码**（注入已还原并断言 check 仍为 0） |
| P22 | 最终回归 | **PASS（本机）**：报告见 `docs/REGRESSION-P22.md` —— MoonBit `check --target native` **0 errors**（33 warnings）、wasm-gc 通过；**桌面纯 Node 39 个脚本失败 0**；**抽检 7 个 Electron 脚本全部 exit=0**（5/5、5/5、41/0、41/0、23/0、14/0、27/0）；门禁三项通过；规模 142 commits / 156 js / 39 测试 / 47 verify / 149 IPC。**测试与构建以 CI 为准**（本机 R12 跑不了 test/build） |
| P23 | 版本发布 | **PASS（到第一档）**：`desktop/package.json` → **`0.2.0-alpha`**，说明见 `docs/RELEASE.md`；对照清单三档，当前只到「IDE Core + Agent Read」（Execute/Patch/Verify 的能力与 MockLLM E2E 就绪，但"真实 LLM 修好真实项目"未验 → 按 RULE-01 不标 beta）；**git tag 未打**（对外宣布版本的动作，等确认） |
