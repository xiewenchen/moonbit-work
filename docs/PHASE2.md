...
| R12 | **本机 MoonBit native 工具链故障** | `moon build --target native <可执行目标>` **全部 EXIT=127**（`moonc link-core` 失败，`main.c` 不生成）；`moon check` 正常、`moon test` 零输出。→ 本机跑不起任何 native 服务，Run 链路 E2E 无法在本机验证 | `changes/phase2-p1-run-diagnosis.md`（P1-06） |
