...
**1. 想理解 HTTP 服务端实现的人。** 手上有 RFC 却看不到字节如何变成请求对象；打开 `http/http.mbt` 逐行对照 RFC、运行自带测试（看 `Content-Length` 如何解析、chunked 边界如何防溢出）；**得到的是一本能拆开看的协议实现，不是黑盒。**
...
**3. 刚写完第一版协议栈的安全工程师。** 想知道手写 HTTP 容易踩哪些坑；读 `docs/VULNERABILITIES.md` 的 41 个缺陷记录，用 `sec/` 靶场与 `attack.py`（覆盖请求走私、限流绕过等 8 类）实测修复效果；**得到的是一本真实失败案例集，而不是教科书。**
