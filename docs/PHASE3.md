...
## 22. 查缺补漏：未接线的纯逻辑层（2026-09-26）

用了三轮才把判据修对，**修对之前工具一直在帮我盖住错误**：

| 轮次 | 判据的毛病 | 结果 |
|---|---|---|
| 1 | 正则 `(?<![\w$.])` 把 `x.fn(` 这种**方法调用**也排除了 | 报 116 项，几乎全是误报 |
| 2 | 修了正则，但没区分"被测试用"和"完全没人用" | 仍混在一起 |
| 3 | 只留"**测试用过、产品代码从未调用**" | 34 模块 / 128 函数，抽验 3 个全部为真 |

> **判据错的时候，工具会帮你把错误盖住** —— 这与"假绿"是同一个道理。

### 抽验（不信扫描器，人工核对）

`createTask` / `canStore` / `replaySession` 三个都grep 过：**只在自身模块内使用，产品侧零调用**。

### 已补

`session-view` 的 `exportSession` / `replaySession` 接成会话面板上的「导出 Markdown」：
- 走 `replaySession` 的**白名单**取字段（不是把 session JSON 化）→ 以后新增的内部字段默认不进导出；
- 导出失败如实说；无项目时明确拒绝（不导出空文件）。
- 验证 `verify-session-export.js` **15/0**（真开项目 → 真点按钮 → 断言界面带字符数反馈 →
  断言导出文本是 Markdown 且**不含** `"role":` / `"messages":` 这类内部结构）。

### 未补（如实登记）

**127 个仍未接线**，完整清单在 `docs/UNWIRED-INVENTORY.md`。
其中 Gate G1 相关的三个（`agent-task` / `agent-verify` / `agent-adapter`）与
"真实模型跑一条完整路径"是**同一件事** —— 需要真实 Provider 才能真正验收。

> **不为了"清零数字"而接线**：接了但没人用的代码比没接更糟，因为它看起来是活的。
